...
void applyLowPass(juce::AudioBuffer<float>& buffer, double srate, int numIn, double freq) {
    ...
    for (int channel; channel < numIn; channel++) {
        ...
        for (int sample = 1; sample < buffer.getNumSamples(); sample++) {
            channelData[sample] = alpha * channelData[sample] + (1-alpha) * channelData[sample-1];
        }
    }
}
